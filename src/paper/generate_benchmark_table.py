...
def generate_table_comparison(cfg_dict: DictConfig):
    ...
    for method in cfg.evaluation.methods:
        ...
        try:
            ...
            autoencoder_encoder_time = np.mean(benchmark.get("autoencoder_encoder", [0]))
            encoder_time = np.mean(benchmark.get("encoder", [0]))
            decoder_time = np.mean(benchmark.get("decoder", [0]))
            autoencoder_decoder_time = np.mean(benchmark.get("autoencoder_decoder", [0]))
            if np.isclose(decoder_time, 0):
                decoder_time = None
        except FileNotFoundError:
            print(f"Warning: Could not load benchmark for {method.key}.")
            autoencoder_encoder_time = None
            encoder_time = None
            decoder_time = None
            autoencoder_decoder_time = None
        ...
        table[method.name] = [autoencoder_encoder_time, encoder_time, decoder_time, autoencoder_decoder_time, peak_memory]
    
    table = make_latex_table(
        table,
        ["Autoencoder_Encoding (S)", "Encoding (s)", "Decoding (s)", "Autoencoder_Decoding (s)", "VRAM (GB)"],
        [8, 8, 8, 8, 8],
        [-1, -1, -1, -1, -1],
    )
